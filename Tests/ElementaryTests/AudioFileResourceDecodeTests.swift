...
final class AudioFileResourceDecodeTests: XCTestCase {
    ...
    func testDecodeAudioBufferResourcePreservesPerChannelSampleData() throws {
        let decoded = try decodeAudioBufferResource(fileURL: stereoFixtureURL)

        XCTAssertEqual(decoded.numChannels, 2)
        XCTAssertEqual(decoded.numSamples, 40)

        let left = decoded.samples(forChannel: 0)
        let right = decoded.samples(forChannel: 1)
        XCTAssertEqual(left.count, 40)
        XCTAssertEqual(right.count, 40)
        ...
        XCTAssertEqual(left[1], 1000.0 / 32768.0, accuracy: 0.001)
        XCTAssertEqual(right[1], -1000.0 / 32768.0, accuracy: 0.001)

        // frame 2: left = 2000/32768, right = -2000/32768.
        XCTAssertEqual(left[2], 2000.0 / 32768.0, accuracy: 0.001)
        XCTAssertEqual(right[2], -2000.0 / 32768.0, accuracy: 0.001)
    }
}
